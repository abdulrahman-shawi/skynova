...
export default function ViewWholesaleOrder({
  data,
  products,
}: {
  data: any;
  ...
  const totalShippingExpenses = getWholesaleOrderTotalShippingExpenses(data);
        ...
        <div id="printable-area">
          <div className="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-start mb-8 md:mb-10 pb-6 md:pb-8 border-b-2 border-slate-100">
            <div className="flex flex-wrap items-baseline gap-3">
              ...
              <span className="text-base md:text-2xl font-bold text-slate-400">| فاتورة مبيعات جملة</span>
            </div>
            <div className="text-left space-y-1 text-xs md:text-sm text-slate-500 font-bold">
              ...
              <p>
                تاريخ الإصدار:{' '}
                <span className="text-slate-900 dark:text-white">{new Date(getWholesaleDisplayDate(data)).toLocaleDateString('ar-EG')}</span>
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-6 md:gap-8 mb-8 md:mb-12">
            <div className="p-6 bg-blue-50/50 dark:bg-blue-900/10 rounded-[2rem] border border-blue-100/50">
              <p className="text-xl font-black text-slate-900 dark:text-white">{data.wholesaleCustomer?.name}</p>
